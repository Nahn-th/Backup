...
const createDefaultGenres = () => {
    const defaultGenres = ['Rock', 'Pop', 'Jazz', 'Classical', 'Hip Hop', 'Electronic', 'Country', 'R&B'];
...
export const insertSong = (song) => {
    try {
        const { title, artist_name_string, genre_string, duration, path, cover_image_path } = song;
        db.execute(
            'INSERT INTO Songs (title, artist_name_string, genre_string, duration, path, cover_image_path) VALUES (?, ?, ?, ?, ?, ?)',
            [title, artist_name_string || '', genre_string || '', duration || 0, path, cover_image_path || '']
        );
...
export const updateSong = (id, updates) => {
    try {
        const { title, cover_image_path, artist_name_string, genre_string } = updates;
        const fields = [];
        const values = [];
...
export const getAllPlaylists = () => {
    try {
        const result = db.execute('SELECT * FROM Playlists ORDER BY created_at DESC');
        return result.rows?._array || [];
    } catch (error) {
...
export const createPlaylist = (name, cover_image_path = '') => {
    try {
        db.execute('INSERT INTO Playlists (name, cover_image_path) VALUES (?, ?)', [name, cover_image_path]);
        const result = db.execute('SELECT last_insert_rowid() as id');
        return result.rows?._array?.[0]?.id || null;
    } catch (error) {
...
export const updatePlaylist = (id, name, cover_image_path) => {
    try {
        ...
        values.push(id);
        db.execute(`UPDATE Playlists SET ${fields.join(', ')} WHERE id = ?`, values);
        return true;
    } catch (error) {
...
export const searchPlaylists = (query) => {
    try {
        const searchTerm = `%${query}%`;
        const result = db.execute('SELECT * FROM Playlists WHERE name LIKE ? ORDER BY name', [searchTerm]);
        return result.rows?._array || [];
    } catch (error) {
...
export const createArtist = (name, cover_image_path = '') => {
    try {
        db.execute('INSERT OR IGNORE INTO Artists (name, cover_image_path) VALUES (?, ?)', [name, cover_image_path]);
...
export const searchArtists = (query) => {
    try {
        const searchTerm = `%${query}%`;
        const result = db.execute('SELECT * FROM Artists WHERE name LIKE ? ORDER BY name', [searchTerm]);
        return result.rows?._array || [];
    } catch (error) {
...
export const createGenre = (name, cover_image_path = '') => {
    try {
        db.execute('INSERT OR IGNORE INTO Genres (name, cover_image_path) VALUES (?, ?)', [name, cover_image_path]);
...
export const searchGenres = (query) => {
    try {
        const searchTerm = `%${query}%`;
        const result = db.execute('SELECT * FROM Genres WHERE name LIKE ? ORDER BY name', [searchTerm]);
        return result.rows?._array || [];
    } catch (error) {
...
export const removeSongFromPlaylist = (songId, playlistId) => {
    try {
        db.execute('DELETE FROM SongPlaylists WHERE song_id = ? AND playlist_id = ?', [songId, playlistId]);
        return true;
    } catch (error) {
...
export const linkSongArtist = (songId, artistId) => {
    try {
        db.execute('INSERT OR IGNORE INTO SongArtists (song_id, artist_id) VALUES (?, ?)', [songId, artistId]);
        return true;
    } catch (error) {
...
export const unlinkSongArtist = (songId, artistId) => {
    try {
        db.execute('DELETE FROM SongArtists WHERE song_id = ? AND artist_id = ?', [songId, artistId]);
        return true;
    } catch (error) {
...
export const linkSongGenre = (songId, genreId) => {
    try {
        db.execute('INSERT OR IGNORE INTO SongGenres (song_id, genre_id) VALUES (?, ?)', [songId, genreId]);
        return true;
    } catch (error) {
...
export const unlinkSongGenre = (songId, genreId) => {
    try {
        db.execute('DELETE FROM SongGenres WHERE song_id = ? AND genre_id = ?', [songId, genreId]);
        return true;
    } catch (error) {

...
export const getAllSongs = () => {
  try {
    ...
    // OP SQLite trả về object với rows là array
...
export const insertSong = song => {
  try {
    ...
    try {
      ...
      const existingRows = checkResult.rows?._array || checkResult.rows || [];
      if (existingRows.length > 0) {
        console.log(
          `Song already exists: ${title} (id: ${existingRows[0].id})`,
        );
        return existingRows[0].id;
      }
    } catch (checkError) {
      // If check fails, continue to insert
      console.log('Check error (continuing):', checkError.message);
    }
    ...
    // Try multiple ways to get the inserted ID
    let insertedId = null;

    // Method 1: Check insertResult directly
    if (insertResult && insertResult.insertId) {
      insertedId = insertResult.insertId;
    }

    // Method 2: Query last_insert_rowid()
    if (!insertedId) {
      try {
        const idResult = db.execute('SELECT last_insert_rowid() as id');
        const idRows = idResult.rows?._array || idResult.rows || [];
        if (idRows.length > 0 && idRows[0].id) {
          insertedId = idRows[0].id;
        }
      } catch (idError) {
        console.log('Error getting last_insert_rowid:', idError.message);
      }
    }

    // Method 3: Query by path (fallback)
    if (!insertedId) {
      try {
        const pathResult = db.execute('SELECT id FROM Songs WHERE path = ?', [
          path,
        ]);
        const pathRows = pathResult.rows?._array || pathResult.rows || [];
        if (pathRows.length > 0 && pathRows[0].id) {
          insertedId = pathRows[0].id;
        }
      } catch (pathError) {
        console.log('Error querying by path:', pathError.message);
      }
    }

    if (insertedId) {
      console.log(`✅ Inserted song: ${title} (id: ${insertedId})`);
    } else {
      console.log(`⚠️ Inserted song but no ID: ${title}`);
    }

    return insertedId;
  } catch (error) {
    console.error('❌ Error inserting song:', error);
    console.error('Song data:', { title: song.title, path: song.path });
    return null;
  }
};
...
export const createPlaylist = (name, cover_image_path = '') => {
  try {
    ...
    return result.rows?.[0]?.id || result.rows?._array?.[0]?.id || null;
  } catch (error) {

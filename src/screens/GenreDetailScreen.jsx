...
const GenreDetailScreen = () => {
  ...
  const loadSongs = () => {
    const genreSongs = getSongsByGenre(genre.id);
    setSongs(genreSongs);
  };
  ...
  const handleLongPress = song => {
    Alert.alert(song.title, 'Choose an action', [
      {
        text: 'Remove from Genre',
        onPress: () => {
          unlinkSongGenre(song.id, genre.id);
          loadSongs();
        },
        style: 'destructive',
      },
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.headerContent}>
            {genre.cover_image_path ? (
              <Image
                source={{ uri: genre.cover_image_path }}
            ...
            <Text style={[styles.genreName, { color: colors.textPrimary }]}>
              {genre.name}
            </Text>
